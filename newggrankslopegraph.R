# Modified from: newggslopegraph.R by Chuck Powell 2018
...
#'   DataLabelLineSize = .5
#' )
newggrankslopegraph <- function(dataframe,
                                Times,
                                Measurement,
                                Grouping,
                                Data.label = NULL,
  ...
  dataframe %>%
    ...
    geom_text(
      #data = . %>% filter(UQ(Times) == min(UQ(Times))),
    ...
    geom_text(
      data = . %>% filter(UQ(Times) == max(UQ(Times))),
      aes_(label = Grouping),
      hjust = "left",
      #box.padding = 0.10,
      #point.padding = 0.10,

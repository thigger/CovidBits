...
library(dplyr)
library(tidyr)
triage_nos <- read_csv("C:/Documents/Work/202007 covid visualisation/NHS Pathways Covid-19 data latest.csv")
...
triage_nos$x<-triage_nos$TriageCount
...
print(
ggplot(test,aes(x=date,y=x,fill=ageband)) + 
  geom_area(size=1,colour="black") + 
  ggtitle("National 111 Triages for COVID-19") + 
  theme_classic(base_size=18) + 
  theme(axis.title.y=element_blank()) + 
  labs(fill="Age Band",x="Date")
)
...
print(
  ggplot(test,aes(x=date,y=x,fill=ageband)) + 
    geom_area(size=1,colour="black") + 
    ggtitle("Bradford 111 Triages for COVID-19") + 
    theme_classic(base_size=18) + 
    theme(axis.title.y=element_blank()) + 
    labs(fill="Age Band",x="Date")
)